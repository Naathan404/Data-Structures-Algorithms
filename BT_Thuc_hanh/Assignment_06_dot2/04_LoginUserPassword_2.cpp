...
int HF(HASHTABLE H, string username) {
    ...
    for (int i = 0; i < username.length(); i++) {
        hash = (hash * 37 + username[i]) % H.M;
    }
    return (int)hash;
}


int HF_LinearProbing(int h, int i, int M)
{
    return (h + i) % M;
}
...
void Search(HASHTABLE H, string username)
{
    ...
    while(i < H.M)
    {
        ...
        if(H.table[pos].flag == OCCUPIED && H.table[pos].key.username == username)
        {
            for(int j = 0; j < H.table[pos].key.password.size(); j++)
            {
                cout << H.table[pos].key.password[j] << " ";
            }
            cout << endl;
            return;
        }
